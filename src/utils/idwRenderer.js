...
// the main thread for one long stretch.
...
export async function renderIDWToCanvas(data, property, bounds, width, height) {
    ...
    // ─── VALUE CAP ────────────────────────────────────────────────────
    // The raw max can be dominated by outliers (e.g. Meghalaya ~5119 mm
    // in the rainfall dataset) which crushes the whole color palette
    // into a flat blue. Cap the effective max so the gradient actually
    // spreads across the meaningful data range.
    const CAP = 2500;
    const effectiveMax = Math.min(maxVal, CAP);
    ...
    const points = data
        .map(item => {
            ...
            // Clamp the value to the cap so anything above CAP maps to
            // the top of the gradient instead of extending the scale.
