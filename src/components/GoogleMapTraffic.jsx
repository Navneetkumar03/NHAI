...
export default function GoogleMapComponent() {
  ...
  const flyoverButtons = useMemo(() => {
    if (!flyovers || flyovers.length === 0) return [];

    return flyovers
      .map((flyover) => {
        const point = flyover.namedPoints?.[0];

        if (!point) return null;

        return {
          name: formatPointName(point.name), // backend name
          value: flyover.type, // F1, F2, F3, F4 internally
          color: getFlyoverColor(flyover.id - 1),
        };
      })
      .filter(Boolean);
  }, [flyovers]);
  ...
  const zoomToFlyover = (flyoverName) => {
    ...
    if (!flyover || !mapRef.current) return;
  ...
  useEffect(() => {
    if (
      flyoverButtons.length > 0 &&
      activeFlyoverId === null &&
      mapRef.current
    ) {
      const firstFlyover = flyoverButtons[0];
  ...
  return (
    ...
    "
          ...
          >
            {flyoverButtons.map((flyover) => {
              const isActive = activeFlyoverId === flyover.value;
