...
export const IDWLeafletLayer = L.Layer.extend({
  ...
  onAdd(map) {
    ...
    map.on("move zoom", this._repositionCanvas, this);
    map.on("zoomanim", this._onZoomAnim, this);
    map.on("moveend zoomend", this._onViewportSettled, this); 

    this._render();
  },

  onRemove(map) {
    map.off("move zoom", this._repositionCanvas, this);
    map.off("zoomanim", this._onZoomAnim, this);
    map.off("moveend zoomend", this._onViewportSettled, this); 
  ...
  _onViewportSettled() {
    ...
    this._viewportDebounce = setTimeout(() => this._render(), 150);
  },
  // ── Public API ─────────────────────────────────────────────────────────────
  ...
  preRenderAllMonths: async function (
    allMonthlyData,
    property,
    layerPropertyMap,
  ) {
    ...
    for (const monthKey of uniqueMonths) {
      const monthData = allMonthlyData.filter(
        (item) =>
      ...
      try {
        ...
        if (this._clipPolygon && this.options.fillViewport === false) {
          try {
  ...
  preRenderAllLayers: async function (
    allMonthlyData,
    properties,
    layerPropertyMap,
  ) {
    ...
    // console.log(
    //   `🔥 Pre-rendering ALL layers (${properties.join(", ")}) for ${uniqueMonths.length} months...`,
    // );
    ...
    // console.log(
    //   "🎉 All layers × all months pre-rendered — every switch is now instant",
    // );
  },
  _getViewportGeoBounds(paddingRatio = IDW_BOUNDS_PADDING_RATIO) {
  ...
  _computeRenderBoundsFromData(data) {
    const viewportBounds = this._getViewportGeoBounds();
    if (viewportBounds) return viewportBounds;

    // Fallback: derive from the data passed in, same as before.
...
if (this._clipPolygon && this.options.fillViewport === false) {
  ...
  _computeRenderBounds() {
    ...
    // Fallback (e.g. layer not yet attached to a map): use data extent
    // so pre-render calls made before onAdd() still work.
  ...
  _computeResolution(geoBounds) {
    ...
    const avgLatRad =
      ((geoBounds.minLat + geoBounds.maxLat) / 2) * (Math.PI / 180);
...
_deriveCacheKey() {
    ...
    const vb = this._getViewportGeoBounds(0);
    if (!vb) return base;

    const r = (v) => Math.round(v * 1000) / 1000;
    const viewportFingerprint = `${r(vb.minLat)},${r(vb.maxLat)},${r(vb.minLng)},${r(vb.maxLng)}`;

    return `${base}::vp(${viewportFingerprint})`;
},
  ...
  _applyPolygonClip(canvas, boundsObj) {
    return new Promise((resolve) => {
      try {
        ...
        const polygonRings = this._getPolygonPixelRings(
          boundsObj,
          width,
          height,
        );
    ...
    try {
      if (input.type === "FeatureCollection")
        return (input.features || []).flatMap((f) =>
          this._extractRings(f.geometry),
        );
      if (input.type === "Feature") return this._extractRings(input.geometry);
  ...
  // ── Public shims ──────────────────────────────────────────────────────────
  isVisible() {
    return !!this._map && !!this._geoBounds;
  },
  getData() {
    return this._data;
  },
