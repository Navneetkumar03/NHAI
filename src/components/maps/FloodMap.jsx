...
const TILE_LAYER_URL =
    "https://mlinfomap.org/nhaiapi/tiles/flood/{z}/{x}/{y}.png";
...
function FloodLegend() {
    return (
        // z-[500] was below Leaflet's own marker/popup panes (600/700), so
        // markers or popups drifting under this corner could render on top
        // of the legend and hide it. Raised well above every Leaflet pane
        // and control (max ~1000) so it always stays visible.
        <div className="absolute bottom-3 right-3 z-[9999] bg-white/95 backdrop-blur-sm rounded-md shadow-md border border-gray-200 px-3 py-2 max-w-[180px]">
            ...
            <div className="flex flex-col gap-1">
                <div className="flex items-center gap-2">
                    <span
                        ...
                        style={{
                            backgroundColor: `rgb(${FLOOD_COLORS.value1.color.join(",")})`,
                        }}
                    />
                ...
                <div className="flex items-center gap-2">
                    <span
                        className="w-3 h-3 rounded-sm flex-shrink-0 border border-gray-200"
                        style={{
                            backgroundColor: `rgb(${FLOOD_COLORS.value2.color.join(",")})`,
                        }}
                    />
                ...
                <div className="flex items-center gap-2">
                    <span
                        className="w-3 h-3 rounded-sm flex-shrink-0 border border-gray-200"
                        style={{
                            backgroundColor: `rgb(${FLOOD_COLORS.value3.color.join(",")})`,
                        }}
                    />
                ...
                <div className="flex items-center gap-2">
                    <span
                        className="w-3 h-3 rounded-sm flex-shrink-0 border border-gray-200"
                        style={{
                            backgroundColor: `rgb(${FLOOD_COLORS.value4.color.join(",")})`,
                        }}
                    />
                    <span className="text-[10px] text-gray-600 leading-tight">
                        Very High
                ...
                <div className="flex items-center gap-2">
                    <span
                        className="w-3 h-3 rounded-sm flex-shrink-0 border border-gray-200"
                        style={{
                            backgroundColor: `rgb(${FLOOD_COLORS.value5.color.join(",")})`,
                        }}
                    />
                    <span className="text-[10px] text-gray-600 leading-tight">
                        Extreme
...
export default function FloodMap({
    mapCenter = DEFAULT_FLOOD_MAP_CENTER,
    isActive = true,
}) {
                    ...
                    try {
                        ...
                        if (flyover.geojson) {
                            try {
                                ...
                                console.error(
                                    `[FloodMap] Error adding flyover layer for ${displayName}:`,
                                    err,
                                );
                            }
                        }
                        ...
                        if (flyover.namedPoints && flyover.namedPoints.length > 0) {
                            flyover.namedPoints.forEach((point) => {
                                ...
                                `);
                                    ...
                                    console.error(
                                        `[FloodMap] Error adding marker for ${point.name}:`,
                                        err,
                                    );
                                }
                            });
    ...
    return (
        <div className="flex flex-col h-full w-full" ref={fullscreenContainerRef}>
            ...
            >
                ...
                {isLayerAdded && (
                    <div
                        // z-[500] previously sat below Leaflet's own marker
                        // (600) and popup (700) panes, so a flyover marker
                        // or an open popup near this corner could render on
                        // top of the panel and make it unreadable/invisible.
                        // Raised above every Leaflet pane and control.
                        className="absolute left-2.5 z-[9999]"
            ...
            `}
                ...
                {(loading || flyoversLoading) && (
                    <div className="absolute inset-0 flex items-center justify-center bg-white/80 backdrop-blur-sm z-[9999]">
                ...
                {error && (
                    <div className="absolute top-3 left-1/2 -translate-x-1/2 z-[9999] bg-red-50 border border-red-200 text-red-700 text-sm px-4 py-3 rounded-lg flex items-center gap-2 shadow-lg max-w-md">
